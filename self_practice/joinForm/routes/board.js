...
router.post('/write', upload.single('img'), function(req,res,next){
    ...
    var img = req.file;
    var data = [creator_id, title, content, passwd];

    pool.getConnection(function (err, connection){
        var sqlForInsertBoard = "insert into board(creator_id, title, content, passwd) values(?,?,?,?)";
        connection.query(sqlForInsertBoard, data, function (err, result){
            if (err) console.error("err : " + err);
            console.log("rows : " + JSON.stringify(result));
            res.redirect('/board');
            fs.rename('public/images/' + img.filename, 'public/images/' + result.insertId, function(err){
                if (err) throw err;
...
router.post('/delete/:idx', function(req, res, next){
    ...
    pool.getConnection(function(err, connection){
        ...
        connection.query(sql, [idx], function(err, rows){
            if(err) console.error(err);
            console.log("1개 항목 delete : ", rows);
            if(fs.exists('public/images/' + idx)){
                fs.unlink('public/images/' + idx, function(err){
                    if (err) throw err;
                    console.log('successfully deleted', idx);
                });
            }
            res.redirect('/board');
            connection.release();
